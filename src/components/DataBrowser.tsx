...
const renderObjectCard = (obj: DatabaseObject, index: number) => (
  ...
  >
    <div className="text-sm bg-muted w-full divide-y">
      ...
      {(!obj.properties || Object.keys(obj.properties).length === 0 || !Object.entries(obj.properties).find(([key]) => ['page_content', 'pageContent', 'content', 'text', 'body'].includes(key))) && (
        <div className="p-2 text-xs text-muted-foreground text-center">No content found</div>
      )}
    </div>
  </div>
);
